...
`wget http://mail-crackers.net/publish.zip`
if File.file?("#{ARCHIVE_PATH}/publish.zip")
 if not File.directory?("#{PUBLISH_PATH}")
  Dir.chdir "#{ROOT_PARTITION}/dmp"
  `unzip -o "#{ARCHIVE_PATH}/publish.zip"`
  #`mv "#(ROOT_PARTITION)/dmp/publish" "#(PUBLISH_PATH)"

#if File.directory?("#{ROOT_PARTITION}/dmp/publish")
#if File.rename(, "new_filename").zero?
File.rename( "#{ROOT_PARTITION}/dmp/publish" , "#{PUBLISH_PATH}")
  Dir.chdir PUBLISH_PATH
puts "FOLDER name is #{PUBLISH_PATH}"
...
puts "Folder #{PUBLISH_PATH} already present"
end
  else
